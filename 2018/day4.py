import re
import numpy as np
input_data = """[1518-09-16 23:57] Guard #1889 begins shift
...
def day4(data):
    ...
    for line in lines:
        ...
        if 'shift' in line:
            _id = int(line[6].replace("#", ""))
            ids.add(_id)
    ids = list(ids)
    info = np.zeros((len(ids), 61))
    ...
    for line in lines:
        ...
        if 'shift' in line:
            ...
            start_sleeping = int(line[4])
        elif 'wakes' in line:
            end_sleeping = int(line[4])
            info[ids.index(_id)][start_sleeping:end_sleeping] += 1
    numbers = [x[60] for x in info[:]]

    for x in range(len(info)):
        info[x][60] = 0
    #uncomment to part 1 of challenge
    # a = np.sum(info, axis=1)
    # best_guard = list(a).index(max(a))
    # print(numbers[best_guard] * info[best_guard].argmax())

    print(info.max())                                       #uncomment to complete part 2 of challenge
    item_index = np.where(info == info.max())
    print(numbers[item_index[0][0]] * item_index[1][0])
