...
test = [3,3,1108,-1,8,3,4,3,99,-1,-1]
...
def store_input(tab, value, address):
    tab[address] = value


def show_value(tab, address):
    print(tab[address])
...
def param_mode(opcode):
    modes = list(map(int, list(str(int(opcode/100)))))
    # modes = list(opcode)[:-2]
    while len(modes) < 3:
        modes = [0] + modes
    return modes
...
def intcode(data, opcode, a_pos, b_pos, result_position):
    ...
    if opc == 99:
        ...
        if mode[-2]:
            ...
            b = data[b_pos]
        data[result_position] = do_summation(a, b)
        return 4
        ...
        if mode[-2]:
            ...
            b = data[b_pos]
        data[result_position] = do_multiplication(a, b)
        return 4
        # data[result_position] = do_multiplication(data[a_pos], data[b_pos])
    elif opc == 3:
        store_input(data, 5, a_pos)
        return 2
    elif opc == 4:
        show_value(data, a_pos)
        return 2
    elif opc == 5:
        mode = param_mode(opcode)
        if mode[-1]:
        ...
        if mode[-2]:
            ...
            b = data[b_pos]
        if a != 0:
            return b - it #jump if true sets instruction pointer to value of second parameter
        return 3
    elif opc == 6:
        mode = param_mode(opcode)
        if mode[-1]:
        ...
        if a < b:
            data[result_position] = 1
        else:
            data[result_position] = 0
        return 4
    elif opc == 8:
        mode = param_mode(opcode)
        if mode[-1]:
...
it = 0
    # data[1] = a
    # data[2] = b
while data_input[it] % 100 != 99:
    pointer_move = intcode(data_input, data_input[it], data_input[it + 1], data_input[it + 2], data_input[it + 3])
    if pointer_move == 99:
        break
    it += pointer_move
    # return a, b
...
# print(param_mode(102))
# print([x for x in data_input if len(str(x)) == 5])

# print(data_input.count(3))