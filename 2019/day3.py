...
for cmd in wires[0]:
    if cmd[0] == 'R':
        goto = curx + int(cmd[1:])
        for step in range(curx, goto):
            steps1.append((step, cury))
        curx = goto
    elif cmd[0] == 'L':
        goto = curx - int(cmd[1:])
        for step in range(curx, goto, -1):
            steps1.append((step, cury))
        curx = goto
    elif cmd[0] == 'U':
        goto = cury + int(cmd[1:])
        for step in range(cury, goto):
            steps1.append((curx, step))
        cury = goto
    elif cmd[0] == 'D':
        goto = cury - int(cmd[1:])
        for step in range(cury, goto, -1):
            steps1.append((curx, step))
        cury = goto
    else:
        break
...
common = set(steps1).intersection(steps2)
common.remove((0, 0))
res = 99999
for pair in common:
    (a, b) = pair
...
moves1, moves2 = [], []
for obj in common:
    moves1.append(steps1.index(obj))
    moves2.append(steps2.index(obj))

print(min(list(map(add, moves1, moves2))))